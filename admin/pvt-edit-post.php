...
$query=mysqli_query($con,"update pvttblposts set PostTitle='$posttitle',AditionalInf='$aditional',CategoryId='$catid',SubCategoryId='$subcatid',PostDetails='$postdetails',PostUrl='$url',Is_Active='$status' where id='$postid'");
if($query)
{
...
<?php
$postid=intval($_GET['pid']);
$query=mysqli_query($con,"select pvttblposts.id as postid,pvttblposts.AditionalInf as aditional,pvttblposts.PostImage,pvttblposts.PostTitle as title,pvttblposts.PostDetails,tblcategory.CategoryName as category,tblcategory.id as catid,tblsubcategory.SubCategoryId as subcatid,tblsubcategory.Subcategory as subcategory from pvttblposts left join tblcategory on tblcategory.id=pvttblposts.CategoryId left join tblsubcategory on tblsubcategory.SubCategoryId=pvttblposts.SubCategoryId where pvttblposts.id='$postid' and pvttblposts.Is_Active=1 ");
...
<div class="form-group m-b-20">
<label for="exampleInputEmail1">M&oacute;dulo</label>
<input type="text" class="form-control" id="aditional" value="<?php echo htmlentities($row['aditional']);?>" name="aditional" placeholder="Identificar esta clase como inicio de un m&oacute;dulo u omitir">
</div>
...
<br />
<a href="pvt-change-image.php?pid=<?php echo htmlentities($row['postid']);?>">Actualizar Imagen</a>
</div>
</div>
</div>
<?php } ?>
